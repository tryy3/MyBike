...
| ----------------- | ------------------------------------------- | -------------------------------- | ---------------------------------------------------------------------------------------------------- |
| **Bootstrap**     | `.env` only                                 | Operator / deploy tooling        | `PORT`, `DB_PATH`, `TURSO_*`, `BETTER_AUTH_SECRET`, `CONFIG_ENCRYPTION_KEY`, migration tooling       |
| **Runtime**       | Settings registry + `app_settings` DB table | Admin UI (or GraphQL admin APIs) | Logging, GraphQL timing, Strava webhook client, OAuth providers, Strava integration                  |
...
**Runtime** values live in the registry (`server/src/lib/settings-registry.ts`). Operators change them in Admin → Configuration. Secrets are encrypted at rest using `CONFIG_ENCRYPTION_KEY`. There is **no** automatic importer that copies `.env` into the database — you paste values manually when migrating.
...
1. **Declared env override** (only `BETTER_AUTH_URL` → `betterAuth.baseUrl` and `CLIENT_URL` → `client.url`) — if set, wins over DB
2. **Database override** — value saved in Admin Configuration
3. **Code default** — from the registry definition

**Inherit** short-circuits own DB values for specific integration credentials (see [§4](#4-strava-login-vs-sync--inherit)).
...
| **Restart required** | Process must restart (e.g. logging file transport, OAuth providers, Strava integration, `betterAuth.baseUrl`) |
...
### Runtime — migrate to Admin Configuration

| Registry key                            | Legacy env                        | Effect  | Secret | Env override            |
| --------------------------------------- | --------------------------------- | ------- | ------ | ----------------------- |
...
| `betterAuth.baseUrl`                    | `BETTER_AUTH_URL`                 | restart | no     | yes (`BETTER_AUTH_URL`) |
| `client.url`                            | `CLIENT_URL`                      | restart | no     | yes (`CLIENT_URL`)      |
| `oauth.providers.tsidp.enabled`         | _(new)_                           | restart | no     | —                       |
...
| `oauth.providers.strava.enabled`        | _(new)_                           | restart | no     | —                       |
...
| `integration.strava.enabled`            | _(new)_                           | restart | no     | —                       |
| `integration.strava.inheritCredentials` | _(new)_                           | restart | no     | —                       |
| `integration.strava.clientId`           | `STRAVA_CLIENT_ID`                | restart | no     | —                       |
...
- `oauth.providers.*.enabled` and `integration.strava.enabled` default to **`false`**. Turn them on in Admin after pasting credentials — the server does not infer enabled state from legacy env.
- Undocumented env fallbacks for migrated keys are removed. Only declared env overrides (`BETTER_AUTH_URL`, `CLIENT_URL`) still participate in precedence.
...
4. Open **Settings → Admin → Configuration**.
5. For each legacy env var in [§2](#2-legacy-env--registry-key-map), copy the current value into the matching registry field and save.
...
8. **Restart** the MyBike server if any restart-required settings changed (OAuth, integration, logging file transport, auth URLs). Hot-reload settings apply without restart.
9. Verify behavior: logging level, Strava login, activity sync, webhook polling.
10. **Remove migrated keys from `.env`** on the MyBike server. Keep bootstrap vars and any intentional env overrides (`BETTER_AUTH_URL`, `CLIENT_URL` if you rely on deploy-time overrides).
11. Leave proxy-host env vars on the **strava-webhook-proxy** deployment unchanged.

### Rollback

If something breaks, you can temporarily restore env overrides for `BETTER_AUTH_URL` and `CLIENT_URL` only. Other migrated keys no longer read from `.env` — fix values in Admin Configuration instead.
...
3. **Optional metadata**
   - `envOverride: { varName: "..." }` only if deploy-time env override is intentional (rare).
...
8. **Tests** — `server/src/test/` for precedence, inherit, secret encryption, and consumer wiring.

**Do not** add boot-time env importers. Operators migrate manually via Admin Configuration.
...
| `server/src/lib/settings-registry.ts`                                      | Definitions and defaults                      |
| `server/src/services/app-settings.ts`                                      | DB read/write, encryption, inherit resolution |
