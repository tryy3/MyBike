...
- **Admin runtime config:** bootstrap-only values (`PORT`, DB/Turso, `BETTER_AUTH_SECRET`, `CONFIG_ENCRYPTION_KEY`) stay in `.env`; admin-edited runtime settings live in `app_settings`.
  ...
  `BETTER_AUTH_URL` and `CLIENT_URL` remain optional env overrides for their runtime settings.
