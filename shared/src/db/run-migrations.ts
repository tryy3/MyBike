...
/** Minimal Drizzle SQLite surface used by the migrator (local or remote). */
...
export type RunDrizzleMigrationsOptions = {
  ...
  /** Called when a journaled migration is re-applied because CREATE TABLE objects are missing. */
  onRepair?: (info: { migration: string | undefined; tables: string[] }) => void;
};
...
export async function runDrizzleMigrations(
  ...
  await repairIncompleteAppliedMigrations(db, migrations, dbMigrationRows, options.onRepair);
...
export async function ensureMigrationsTableV1(
  db: MigratorDb,
  migrationsTable: string,
): Promise<void> {
...
async function repairIncompleteAppliedMigrations(
  ...
  localMigrations: LocalMigration[],
  dbRows: DbMigrationRow[],
  onRepair?: RunDrizzleMigrationsOptions["onRepair"],
): Promise<void> {
